...
export const createStudentReport = async (req, res) => {
  try {
    ...
    if (!student || student.role !== "student") {
      return res
        .status(403)
        .json({ message: "Only students can generate reports" });
    }
...
export const getAllReports = async (req, res) => {
  try {
    ...
    const reports = await Report.find()
      ...
      .populate("exercise", "title subject");
...
export const getStudentReports = async (req, res) => {
  try {
    ...
    const reports = await Report.find({ student: studentId }).populate(
      "exercise",
      "title subject"
    );
    res.status(200).json(reports);
...
// Get Reports for a Specific exercise (Only Teachers & school)
export const getExerciseReports = async (req, res) => {
  try {
    ...
    const reports = await Report.find({ exercise: exerciseId }).populate(
      "student",
      "name email"
    );
    res.status(200).json(reports);
...
export const getReportById = async (req, res) => {
  try {
    ...
    if (
      req.school.role !== "school" &&
      report.student.toString() !== req.student.id
    ) {
      return res.status(403).json({ message: "Access denied" });
    }
